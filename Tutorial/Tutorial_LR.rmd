...
### Combine results 
```{r}
paras = paras_CR %>% right_join(paras_LR, by=c("feature","metadata"))
```
...
# P-value: Vanilla 
parasVanilla = paras
parasVanilla$pval = pmin(parasVanilla$pval_CR, parasVanilla$pval_LR)
...
data = output[which(output$pval_LR<=0.05 & output$pval_CR>=0.5),][1:10,]
...
```{r, warning=FALSE}
# P-value: CCT
parasCCT = paras_CR %>% right_join(paras_LR, by=c("feature","metadata"))
parasCCT$pval = combinePvalCCT(parasCCT$pval_CR, parasCCT$pval_LR)
