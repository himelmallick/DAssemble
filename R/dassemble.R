...
DAssemble <- function(features,
  ...
  norm_method <- match.arg(enhancer_norm, choices = c("none", "scran", "tmm", "clr"))
  ...
  if (length(enh_list) > 0L) {
    feat_normalized <- DAssemble_normalize_features(
      features          = features,
      metadata          = metadata,
      method            = enhancer_norm,
      return_normalized = FALSE
    )
    ...
    for (e in enh_list) {
      ...
      tmp <- do.call(fun, list(feat_normalized$features, feat_normalized$metadata, expVar))
      ...
      if (is.null(res)) {
        # No core: start from the first enhancer
        res <- tmp
      } else {
