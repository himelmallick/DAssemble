...
DAssemble_normalize_features <- function(features,
  ...
  if (return_normalized && method %in% c("scran", "TMM")) {
    X <- X / sf
  }
  ...
  features_out <- switch(
    method,
    "CLR"  = as.data.frame(X),                # always transformed
    "none" = features,                         # untouched
    if (return_normalized) as.data.frame(X) else features
  )
